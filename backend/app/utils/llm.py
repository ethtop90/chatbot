...
def create_follow_up_chain(rag_chain):
    follow_up_q_prompt = """
    あなたの知識に基づいて、適切なフォローアップ質問を2～4個提案します。
    このような質問は、答えを出さずに、ユーザーの関心に応じてリスト形式で提示してください。重複や些細な質問は避けてください。
    個々のフォローアップ質問は10文字を超えてはいけません。
    ...
    follow_up_prompt_template = ChatPromptTemplate.from_messages(
        [
            ...
            MessagesPlaceholder(variable_name="chat_history"),
            ("human", "与えられた質問{question}をもとに、元の質問の文脈に応じて、与えられた質問{question}に関連するフォローアップ質問を2～4個作成してください。質問は回答を含まず、リスト形式で提示してください。フォローアップの質問は、元の質問で言及されたトピックに関連するもので、重複や些細な質問は避けてください。 回答は日本語でお願いします。"),
        ]
    )
